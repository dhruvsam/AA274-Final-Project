...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        for i in range(-1,2):
            for j in range(-1,2):
                neigh = (x[0] + i*self.resolution, x[1] + j*self.resolution)
                if self.is_free(neigh) and neigh != x:

                    neighbours.append(self.snap_to_grid(neigh))
        return neighbours
        ...
        while len(self.open_set)>0:
            ...
            for neigh in self.get_neighbors(x_curr):
                if neigh in self.closed_set:
                    continue
                tentative_g_score  =  self.g_score[x_curr] + self.distance(x_curr, neigh)
                ...
                self.f_score[neigh] = tentative_g_score
                self.g_score[neigh] = tentative_g_score + self.distance(neigh,self.x_goal)
        return False
